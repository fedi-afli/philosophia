...
export class AvailabilityPickerComponent implements OnInit {
  ...
  // grid[dayIndex][slotIndex] = true → available (green), default false = unavailable
  ...
  constructor(private availabilityService: TeacherAvailabilityService) {}
  ...
  ngOnInit(): void {
    this.availabilityService.getAvailability().subscribe({
      ...
      error: () => {
        this.errorMessage = "Impossible de charger vos disponibilités.";
        this.isLoading = false;
      },
    });
  }

  private applyExisting(existing: AvailabilityRangeDto[]): void {
  ...
  removeRange(range: AvailabilityRange): void {
  ...
  save(): void {
    ...
    const payload: AvailabilityRangeDto[] = this.ranges.map((r) => ({
      dayOfWeek: r.dayOfWeek,
      startTime: r.startTime,
      endTime: r.endTime,
    }));

    this.availabilityService.updateAvailability(payload).subscribe({
      next: () => {
        this.isSaving = false;
